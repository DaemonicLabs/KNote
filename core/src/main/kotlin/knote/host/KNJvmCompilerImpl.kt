...
import org.jetbrains.kotlin.cli.jvm.compiler.collectScriptsCompilationDependencies
...
class KNJvmCompilerImpl(val hostConfiguration: ScriptingHostConfiguration) : KJvmCompilerProxy, KLogging() {
        ...
        try {
            ...
            val kotlinCompilerConfiguration = org.jetbrains.kotlin.config.CompilerConfiguration().apply {
                ...
                put(
                    ...
                    "kotlin-script"
                ) // TODO" take meaningful and valid name from somewhere
            ...
            val analyzerWithCompilerReport =
                AnalyzerWithCompilerReport(messageCollector, environment.configuration.languageVersionSettings)

            val psiFileFactory: PsiFileFactoryImpl =
                PsiFileFactory.getInstance(environment.project) as PsiFileFactoryImpl
            val scriptText = getMergedScriptText(script, updatedConfiguration)
            val scriptFileName =
                script.name ?: "script.${updatedConfiguration[ScriptCompilationConfiguration.fileExtension]}"

            val virtualFile =
                ScriptLightVirtualFile(scriptFileName, (script as? FileScriptSource)?.file?.path, scriptText)

            val ktScriptCallbackHandler = scriptCompilationConfiguration[ktScriptCallbackHandler]

            val psiFile: KtFile =
                psiFileFactory.trySetupPsiForFile(virtualFile, KotlinLanguage.INSTANCE, true, false) as KtFile?
                    ?: return failure("Unable to make PSI file from script").also {
                        ktScriptCallbackHandler?.invoke(null)
                    }

            val ktScript = psiFile.declarations.firstIsInstanceOrNull<KtScript>()
                ?: return failure("Not a script file").also {
                    ktScriptCallbackHandler?.invoke(null)
                }
//            ktScriptCallbackHandler?.invoke(ktScript)
            ktScriptCallbackHandler?.invoke(psiFile)
